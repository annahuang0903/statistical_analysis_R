...
#  Instead of size, the value of another continuous variable can be     #
#  represented by the size of the point.                                #
...
ggplot(uspopage, aes(x=Year, y=Thousands, colour=AgeGroup)) + geom_line()
...
hist1 <- ggplot(faithful, aes(x=faithful$waiting))
...
pf(17, df1=3, df2=25)
pchisq(7, df=3)
pbinom(5, size=10, prob=0.3) #include 5
ppois(3, lambda=2.5)
...
DavisScatter2 + labs(title="Weights of individuals who regularly exercise",
                     x="Measured weight", y="Reported weight") +
    labs(colour="Gender") + 
    scale_colour_discrete(limits=c("M","F"))
