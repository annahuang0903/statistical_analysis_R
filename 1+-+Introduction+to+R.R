...
## Loading a package
library("MASS") #put at top
require("MASS")
library("MASS");require("MASS")
##############
#  Commands  #
##############
...
#  window or you can use the keyboard shortcut Ctrl + R.                #
#                                                                       #
...
Log(2.7)  #case sensitive;
exp(1)
sqrt(9)
factorial(3)
abs(-5)
sum(2,4,6)
floor(5.7) #round down
ceiling(5.7) #round up
trunc(5.7) #truncate
round(5.7,0) #round to nearest integer
round(5.727,2) #round to nearest 2 decimal place
...
char_vect <- c("Adam", "Gwen", "Alicia", "Blake")
...
as.numeric(char_vect) #NA NA NA NA
...
seq(1,5, length=5) #need to specify length=
seq(1,5, length=9)
...
coach <- char_vect
coach  # Season 12 coaches

coach[2] <- "Miley" 
coach  # Season 11 coaches

coach[2:3] <- c("Pharrell", "Christina")
coach  # Season 10 coaches
...
coach10 <- coach11
coach10[2:3] <- c("Pharrell", "Christina")
...
#                                                                       #
#  Vectors can also be used as inputs to several functions.             #
...
summary(x)
cumsum(y) #start with first, add up the values
...
## Are any elements greater than 3?
x > 3

## Are any elements greater than or equal to 3?
x >= 3

## Are any elements exactly equal to 3?
x == 3

## Are any elements not equal to 3?
x != 3

#  Multiple logical expressions can be combined using either and or or. #

## Are any elements greater than 2 and equal to 3?
x > 2 & x == 3 

## Are any elements greater than 3 or less than 2?
x > 3 | x < 2
...
matrix(c(11,2,1,11,3,0,0,5), nrow=2, ncol=4, byrow=TRUE)
matrix(c(11,2,1,11,3,0,0,5), nrow=4, ncol=2, byrow=FALSE)

matrix(c(11,2,1,11,3,0,0,5), nrow=4, ncol=2, byrow=TRUE)
matrix(c(11,2,1,11,3,0,0,5), nrow=2, ncol=4, byrow=FALSE)
...
rbind(x,y) #combine rows
cbind(x,y) #combine columns
...
dim(coach_stat) #rows*columns
 
#  The dimension function can also be used to turn a vector into a      #
#  matrix.                                                              #

coach_vect <- c("Adam","Gwen","Alicia","Blake",11,2,1,11,3,0,0,5)
...
#  In order to apply a function to a portion of a matrix, you can use   #
#  the apply function, apply(). This function uses three inputs -       #
#  matrix, margin, and function. The matrix input specifies the matrix  #
#  to which you would like the function to be applied, margin specifies #
#  whether you want the function applied to the rows, columns, or       #
#  individual values, and function specifies the function to be         #
#  applied. Since the apply function separates the matrix into a        #
#  collection of vectors, users can use a pre-existing R function that  #
#  applies to vectors or write their own function.                      #

## Sum the rows of matrix A
apply(A,1,sum) #1-margin of rows
...
#  heterogeneous data. Lists are the most flexible type of object in    #
...
voice_data <- list(Name=char_vect, Seasons=x, Host="Carson", 
                   Former=c("Christina","Ceelo","Usher","Shakira","Pharrell","Miley"), 
                   F_Seasons=c(6,4,2,2,4,1))
...
voice_data[4:5] #create a new list
...
#                                                                       #
#  If the data file comma separated (.csv), the function read.csv()     #
...
#  There is a package available for reading in an Excel file (.xlsx)    #
#  called xlsx. This package contains the function read.xlsx(). This    #
#  function takes two inputs - file and sheet index.                    #

install.packages("xlsx")
library(xlsx)
read.xlsx("Data1.xlsx", sheetIndex=1)
